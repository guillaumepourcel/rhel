"""
This module implements a `Dataloader` class for loading and batching time series data.

Input data should be a `jnp.ndarray` of shape `(n_samples, n_timesteps, n_features)`.

Data can be stored as a NumPy array to save GPU memory, with each batch converted to a JAX NumPy array
when `inmemory=False` is specified.
